...
async def get_paper_recommendations_single(
    ...
    try:
        ...
        result = await _client().recommend_for_paper(request)
        track_papers(result, "get_paper_recommendations_single")
        return result
...
async def get_paper_recommendations_multi(
    ...
    try:
        ...
        result = await _client().recommend_from_papers(request)
        track_papers(result, "get_paper_recommendations_multi")
        return result
    except S2ValidationError as exc:
        return s2_exception_to_error_response(exc)
    except S2ApiError as exc:
